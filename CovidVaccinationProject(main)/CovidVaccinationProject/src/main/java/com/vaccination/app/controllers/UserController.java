...
public class UserController {
	...
	@RequestMapping("/register") // URL - localhost:8084/user/register Method - GET
	...
	@RequestMapping(value="/changestatus/{aadhaar}",method = RequestMethod.POST)
	public String changeStatusOfUser(@PathVariable("aadhaar")String aadhaar,Model model,User user) {
		
		User getUser=userService.getUserByAadhaar(aadhaar);
		getUser.setStatus(user.getStatus());
		getUser.setCentre(user.getCentre());
		
		User user1 = userService.changeStatusOfUser(getUser);
		if(user1!=null) {
			return "redirect:/user/get/all";
		}else {
			List<VaccinationCentre> available_centres = vaccinationCentreService.getAvailableCentres();
