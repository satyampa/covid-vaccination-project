...
public class UserRepository {
	...
	public User changeStatusOfUser(User user) {
		
		EntityManager em=emf.createEntityManager();
		
		TypedQuery<User> tq=em.createNamedQuery("UPDATE USER u SET u.status=:status,u.centre=:centre WHERE u.id=:id", User.class);
		tq.setParameter("status", user.getStatus());
		tq.setParameter("centre", user.getCentre());
